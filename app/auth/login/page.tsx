...
function LoginForm() {
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setError(error instanceof Error ? error.message : "Deu ruim, tenta de novo")
    } finally {
      setIsLoading(false)
    }
  }
  ...
  const handleGoogleLogin = async () => {
    ...
    try {
      const { error } = await supabase.auth.signInWithOAuth({
        provider: "google",
        options: {
          redirectTo: process.env.NEXT_PUBLIC_DEV_SUPABASE_REDIRECT_URL || `${window.location.origin}${redirect}`,
        },
      })
      if (error) throw error
    } catch (error: unknown) {
      setError(error instanceof Error ? error.message : "Deu ruim com o Google")
      setIsLoading(false)
    }
  }
        ...
        <div className="flex flex-col gap-6">
          ...
          <Card className="border-red-900/30">
            <CardHeader>
              ...
              <CardDescription>
                {message
                  ? `Isso aqui precisa de conta (tá tentando acessar ${message})`
                  : "Acessa tua conta aqui (se lembrar a senha)"}
              </CardDescription>
            </CardHeader>
            <CardContent>
              ...
              <form onSubmit={handleLogin}>
                <div className="flex flex-col gap-6">
                  ...
                  <Button type="submit" className="w-full" disabled={isLoading}>
                    {isLoading ? "Tentando entrar..." : "Bora lá"}
                  </Button>
                </div>
          ...
          <div className="text-center">
            <Button variant="ghost" asChild>
              <Link href="/store">Voltar pro site (sem login)</Link>
            </Button>
          </div>
        </div>
      </div>
